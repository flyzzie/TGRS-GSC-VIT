...
def cnn2d(dataset):
    ...
    if dataset == 'sa':
        ...
        model = CNN2D(in_channels=270, n_classes=9)
    elif dataset == 'botswana':
        model = CNN2D(in_channels=145, n_classes=14)
    elif dataset == 'flt':
        model = CNN2D(in_channels=80, n_classes=10)
    return model

from thop import profile

if __name__ == '__main__':
    t = torch.randn(size=(1, 1, 80, 13, 13))
    print("input shape:", t.shape)
    net = cnn2d(dataset='flt')
