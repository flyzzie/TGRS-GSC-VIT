...
def gaht(dataset, patch_size):
    ...
    if dataset == 'sa':
        ...
        model = MyTransformer(img_size=patch_size, in_chans=80, num_classes=10, n_groups=[4, 4, 4], depths=[1, 2, 1])
    elif dataset == 'ksc':
        model = MyTransformer(img_size=patch_size, in_chans=176, num_classes=13, n_groups=[4, 4, 4], depths=[1, 2, 1])
    elif dataset == 'ip':
        model = MyTransformer(img_size=patch_size, in_chans=200, num_classes=16, n_groups=[4, 4, 4], depths=[1, 2, 1])
    elif dataset == 'hus':
        model = MyTransformer(img_size=patch_size, in_chans=144, num_classes=16, n_groups=[4, 4, 4], depths=[1, 2, 1])
    elif dataset == 'MUUFL':
        model = MyTransformer(img_size=patch_size, in_chans=64, num_classes=11, n_groups=[4, 4, 4], depths=[1, 2, 1])
    elif dataset == 'Trento':
        model = MyTransformer(img_size=patch_size, in_chans=63, num_classes=6, n_groups=[4, 4, 4], depths=[1, 2, 1])
    elif dataset == 'botswana':
        model = MyTransformer(img_size=patch_size, in_chans=145, num_classes=14, n_groups=[4, 4, 4], depths=[1, 2, 1])
    return model

from thop  import profile

if __name__ == "__main__":
    t = torch.randn(size=(1, 1, 80, 8, 8))
    print("input shape:", t.shape)
    net = gaht(dataset='flt', patch_size=8)
